...
public class SiteAccountsService {
    ...
    public ResponseEntity createAccaunt(SiteRegistrationRequest regAcc) throws AccountExceptions {
            ...
            try {
                ...
                throw new AccountExceptions("Ошибка зашифровки пароля");
            }
            if(siteAccountsRep.findByLogin(regAcc.getLogin()) != null || siteAccountsRep.findByL2email(regAcc.getL2email()) != null) {
                throw new AccountExceptions("Логин или пароль уже зарегистрированы на сайте");
            }
            try{
    ...
    private boolean verifyPassword(String rawPassword, String storedHash) throws AccountExceptions {
        try {
            MessageDigest md = MessageDigest.getInstance("WHIRLPOOL", "BC");
